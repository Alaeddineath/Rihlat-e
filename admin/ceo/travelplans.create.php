...
if (!isset($_SESSION['email']) || !isset($_SESSION['password']) || !isset($_SESSION['role'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Retrieve the travel plan information from the form
        $name = $_POST['name'];
        $date = $_POST['date'];
        $price = $_POST['price'];
        ...
        // Insert the travel plan information into the database
        $sql = "INSERT INTO travel_plan (name, date, price, duration, availability, description, guide_id, hotel_id, destination) VALUES ('$name', '$date', '$price', '$duration', '$availability', '$description', '$guide_id', '$hotel_id', '$destination')";
        ...
        if ($conn->query($sql) === true) {
            ...
            echo "Error inserting travel plan: " . $conn->error;
        }
    }
?>
<!DOCTYPE html>
                ...
                <div class="col-md-12">
                        <form action="" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="guide_id" required placeholder="Please enter a valid ID">
                            </div>
                            <div class="form-group">
                                <label for="hotel_id">Hotel ID:</label>
                                <input type="number" class="form-control" name="hotel_id" required placeholder="Please enter a valid ID">
                            </div>
                            </div>
                            <div class="form-group">
                                <label for="destination">Destination:</label>
                                <textarea class="form-control" name="destination" rows="5" required></textarea>
                            </div>
